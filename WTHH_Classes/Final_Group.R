...
if(K==1){
  mod <- stan(file="Class_Full.stan", cores = n.cores, chains=n.cores,
              control = list("adapt_delta"=.99))
} else {
  mod <- stan(file="Class_Full.stan", cores = n.cores, chains=n.cores,init=init,
              control = list("adapt_delta"=.99))
}



save.image(paste("Full_Out_", K, ".RData", sep=""))
q()